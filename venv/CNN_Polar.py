...
model = models.Sequential([
    ...
    layers.Conv2D(128, (3, 3), input_shape=(BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE),kernel_regularizer=regularizer),
    layers.BatchNormalization(),
    layers.Activation("relu"),
    layers.MaxPooling2D((2,2)),

    # layers.Conv2D(64, (3,3),activation='relu',input_shape=(IMAGE_SIZE,IMAGE_SIZE) ),
    layers.Conv2D(64, (3, 3),kernel_regularizer=regularizer),
    layers.BatchNormalization(),
    layers.Activation("relu"),
    layers.MaxPooling2D((2,2)),

    # layers.Conv2D(32, (3,3),activation='relu',input_shape=(IMAGE_SIZE,IMAGE_SIZE) ),
    layers.Conv2D(32, (3, 3),kernel_regularizer=regularizer,),
    layers.BatchNormalization(),
    layers.Activation("relu"),
    layers.MaxPooling2D((2,2)),
    ...
    layers.Dense(512,activation='relu',name='dense_map_polar'),
    layers.Dropout(0.2),
...
model_version = len( os.listdir("models_saved_json_polar"))+1
os.makedirs("../venv/models_saved_json_polar/model_{}".format(model_version))
model_json = model.to_json()
with open("../venv/models_saved_json_polar/model_{}/model.json".format(model_version),"w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("../venv/models_saved_json_polar/model_{}/model_weights.h5".format(model_version),overwrite=True)
print("Saved model to disk")

model_version = len( os.listdir("../venv/models_saved_json_polar"))
print("MODELVERSION",model_version)
# load json and create model
json_file = open('../venv/models_saved_json_polar/model_{}/model.json'.format(model_version), 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("../venv/models_saved_json_polar/model_{}/model_weights.h5".format(model_version))
...
plt.savefig("../venv/models_saved_json_polar/model_{}/plot.png".format(model_version))
# plt.imshow()
